...
async function request<T>(method: string, path: string, body?: unknown, query?: Query): Promise<T> {
  ...
  // Un FormData viaja tal cual: el navegador le pone el Content-Type con su separador.
  ...
  try {
    res = await fetch(`/api${path}${qs ? `?${qs}` : ''}`, {
      method,
      headers: body !== undefined && !esArchivo ? { 'Content-Type': 'application/json' } : undefined,
      body: esArchivo ? (body as FormData) : body !== undefined ? JSON.stringify(body) : undefined,
    });
  } catch {
...
export const api = {
  ...
  /** Sube un archivo como multipart, que es lo que espera el interceptor de Nest. */
  subir: <T>(path: string, archivo: File, campo = 'archivo') => {
    const datos = new FormData();
    datos.append(campo, archivo);
    return request<T>('POST', path, datos);
  },
